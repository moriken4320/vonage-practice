const Vonage = function(campaignId, apiKey, sessionId, token, events = {}) {
    ...
    this.initSession = function(){
        if (this.isSupported) {
            ...
            .on("muteForced", function (event) {
                this.debugLog("session muteForced:", event);
            }, this)
            // 自分がセッションに接続されたらディスパッチ
            .on("sessionConnected", function (event) {
            ...
            .on("streamPropertyChanged", function (event) {
                this.debugLog("session streamPropertyChanged:", event);
            }, this);

            if (this.sessionObj) this.sessionObj.connect(this.token, function(error) {
    ...
    this.publish = function(){
        ...
        if (this.sessionObj && this.publisherObj) this.sessionObj.publish(this.publisherObj, function (error) {
            if (error) {
    ...
    this.deviceUpdated = function() {
        ...
        if (!this.videoDeviceList.some((device) => device.deviceId === this.selectedVideoDeviceId)) {
            this.setAudioSource('default');
        }
    }
    ...
    this.screenPublish = async function(){
        ...
        this.screenPublisherObj = OT.initPublisher(this.videoTagId, this.screenOpts, function(error) {
            if (error) {
        ...
        .on("videoDimensionsChanged", function(event) {
